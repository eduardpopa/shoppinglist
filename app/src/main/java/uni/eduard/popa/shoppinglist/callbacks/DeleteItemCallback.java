...
import uni.eduard.popa.shoppinglist.tasks.DeleteItemTask;

public class DeleteItemCallback implements LoaderManager.LoaderCallbacks<Pair<Integer, ItemModel>> {
    Context context;
    ListItemsAdapter adapter;
    public DeleteItemCallback(Context context, ListItemsAdapter adapter){
    ...
    public Loader<Pair<Integer, ItemModel>> onCreateLoader(int id, @Nullable Bundle args)  {
        if(args==null) throw new NullPointerException("Cannot delete null item");
        return new DeleteItemTask(context, args.getParcelable(ListItemsActivity.KEY_ITEM), args.getInt(ListItemsActivity.KEY_POSITION));
    }
    @Override
    public void onLoadFinished(@NonNull Loader<Pair<Integer, ItemModel>> loader, Pair<Integer, ItemModel> data) {
        adapter.removeItem(data.first);
        Toast.makeText(context, R.string.task_item_deleted, Toast.LENGTH_SHORT).show();
    }

...
public class DeleteItemTask  extends AsyncTaskLoader<Pair<Integer, ItemModel>> {
    ItemModel item;
    int position;

    public DeleteItemTask(@NonNull Context context, ItemModel item, int position) {
        super(context);
        this.item = item;
    ...
    public Pair<Integer, ItemModel> loadInBackground() {
        ShoppingListDatabase.getDatabase(getContext()).itemModel().delete(item);
